...
Critter::Critter(int hp, int spd, int str, int lvl, int reward, std::vector<Position> path)
    : hp(hp), speed(spd), strength(str), level(lvl), reward(reward), path(path), positionIndex(0), alive(true), slowed(false), burning(false) {}
...
bool Critter::isSlowed() const {
    return slowed;
}

bool Critter::isBurning() const {
    return burning;
}
...
void Critter::setSlowed(bool result){
    slowed = result;
}

void Critter::setBurning(bool result){
    burning = result;
}
